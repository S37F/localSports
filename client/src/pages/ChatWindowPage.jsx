...
function ChatWindowPage() {
  ...
  const { user } = useAuth();
  const userIdStr = user?._id != null ? String(user._id) : '';
  ...
  useEffect(() => {
    const fetchChatData = async () => {
      try {
        ...
        const currentConv = convsRes.data.data.find(
  ...
  useEffect(() => {
    ...
    socket.on('newMessage', (msg) => {
      ...
      if (String(msg.senderId) !== userIdStr) {
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        <div className="space-y-4 max-w-3xl mx-auto flex flex-col pb-4">
            ...
            {messages.map((msg, idx) => {
                const isMine = String(msg.senderId?._id) === userIdStr;
