...
## Deployment: same repo, two Vercel projects (monorepo)

Use **one Git repository** linked to **two Vercel projects**. In each project, set **Root Directory** so Vercel only builds that app.

### 1) API project (backend)
...
| Output | _none — serverless functions_ |
...
Production URL looks like `https://<project>.vercel.app`. REST routes live under `/api/...` (e.g. `/api/health`).

**Note:** Deploying the API on Vercel uses **serverless** functions. **Socket.IO does not run there**; messages still persist via REST, but realtime push over websockets only works when you run **`node server/index.js`** (local, Docker, or a long‑running Node host).

Configuration files: **`server/vercel.json`** (rewrite all paths → `/api`), **`server/api/index.js`** (handler).
...
| Framework Preset | Vite (optional; `client/vercel.json` also sets outputs) |
| Build Command | `npm run build` |
| Output Directory | `dist` |

Environment variables:

- **`VITE_API_URL`** — full API base including `/api`, e.g. `https://<your-api-project>.vercel.app/api` (no trailing slash after `/api`)

### 3) After deploy

1. Confirm `GET https://<api>.vercel.app/api/health` returns JSON.  
2. Set **`CLIENT_URL`** on the API project to match your deployed frontend URL(s).  
3. Redeploy the client if you change **`VITE_API_URL`**.
